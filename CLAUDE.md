...
- Treat `src/client/noumi.d.ts` as the complete public API type source. It must remain one self-contained declaration file with no imports or re-exports from `scripts/`; keep comments limited to non-obvious public semantics and reuse existing named structures. Public types live under the `Noumi.*` namespace to avoid global-name collisions, while the runtime property remains `window.NoumiBridge`. Keep `noumi-global-contract.typecheck.ts` passing whenever Runtime SDK types change.
- Treat `.noumi/contract.json` as the installed platform interface-contract marker. Ordinary app work must not edit it; explicit interface upgrades use the Static Light Systems Skill updater and then complete typecheck, build, sync, and exact-commit deployment.
