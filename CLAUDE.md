...
- Treat `src/client/noumi.d.ts` as the complete public API type source. It must remain one self-contained declaration file with no imports or re-exports from `scripts/`; keep comments limited to non-obvious public semantics, reuse existing named structures, and omit the redundant `Noumi` prefix from declaration names while preserving the runtime property `window.NoumiBridge`. Keep `noumi-global-contract.typecheck.ts` passing whenever Runtime SDK types change.
